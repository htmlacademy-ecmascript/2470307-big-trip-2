  ...
  return `
    <header class="event__header">
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${dateFrom ? formatDateTime(dateFrom) : ''}" ${isDisabled ? 'disabled' : ''}>
        &mdash;
        <label class="visually-hidden" for="event-end-time-1">To</label>
        <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${dateTo ? formatDateTime(dateTo) : ''}" ${isDisabled ? 'disabled' : ''}>
      </div>
      ...
      <button class="event__reset-btn" type="reset" ${isDeleting ? 'disabled' : ''}>${getResetButtonText()}</button>
